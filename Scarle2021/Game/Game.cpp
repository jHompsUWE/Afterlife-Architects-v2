...
void Game::Initialize(HWND _window, int _width, int _height)
{
    ...
    */
    ...
    m_TPScam = new TPSCamera(0.25f * XM_PI, AR, 1.0f, 10000.0f, Vector3::UnitY, Vector3(0.0f, 10.0f, 50.0f));
    ...
    //Test Sounds
    Loop* loop = new Loop(m_audioEngine.get(), "NightAmbienceSimple_02", 1);
    loop->SetVolume(0.1f);
    loop->Play();
    m_Sounds.push_back(loop);

    TestSound* TS = new TestSound(m_audioEngine.get(), "Explo1", 1);
